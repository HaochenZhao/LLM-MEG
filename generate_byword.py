...
run_ses_num = [0,7,7,6,8,6,7,6,7,6,6]


def main(model_step, layer, model):

    model.eval()
    model.to('cuda')

    model_name = model_step + '/layer' + str(layer)

    save_dir = "/data/zhiang/hidden_states_s/" + model_name

    if not os.path.exists(save_dir + "/total"):
        os.makedirs(save_dir + "/total")
    '''
    if not os.path.exists(save_dir + "/sub001"):
        os.makedirs(save_dir + "/sub001")
    if not os.path.exists(save_dir + "/sub002"):
        os.makedirs(save_dir + "/sub002")
    if not os.path.exists(save_dir + "/sub003"):
        os.makedirs(save_dir + "/sub003")
    '''

    # Open txt file and get lines
    for ses in range(1, 11, 1):
        s_data = np.zeros((0,4096))
        for run in range(1,run_ses_num[ses]+1,1):
            context = []
            txt_path = "/data/zhiang/txt/{:>02d}_{}_t.txt".format(ses, run)
            for index, line in enumerate(open(txt_path)):
                context.append(line)
            context_total = len(context)
            #print(context_total)

            data_tensor = torch.zeros(0,4096).cuda()

            print('start running... at', maket(int(time.time())))
            print("model_step: ", model_step, "layer: ", layer, "session: ", ses, "run: ", run)
            with tqdm(total=context_total) as pbar:
                txt = ''
                for i in range(context_total):
                    txt = txt + context[i].replace('\n', '')
            ...
            s_data = np.concatenate((s_data, data_array), axis=0)
        save_path = save_dir + "/total/hs_{}.npy".format(ses)
        
        hs = np.delete(s_data, modi_lists[ses], axis=0)
        np.save(save_path, hs)
        '''
        for sub in range(1,4,1):
            wd_id_droppeds = wd_id_ds[sub]
            save_path = save_dir + "/sub{:03d}/hs_{}.npy".format(sub, ses)
            hs = np.delete(s_data, wd_id_droppeds[ses], axis=0)
            print("model_step: ", model_step, "layer: ", layer, "sub: ", sub, "session: ", ses)
            print(hs.shape)
            np.save(save_path, hs)
        '''
...
def get_layer_states_by_text(input_text):
    ...
    with torch.no_grad(): # No need to compute gradients
        ...
        layer_activations = torch.zeros(words_num, target_hidden_layers.shape[1]).cuda()
        if is_display:    
...
def filter(step, layer,sub):


    model_name = step + '/layer' + str(layer)
    save_dir = "/data/zhiang/hidden_states_ns/" + model_name
    ...
    print("model_step: ", model_step, "layer: ", layer, "sub: ", sub)
    with tqdm(total=10) as pbar:
        for ses in range(1,11,1):
            hs = np.zeros((0,4096))
            for run in range(1,run_ses_num[ses]+1,1):
                if ses in [2,3,4,10]:
                    run_path = save_dir + "/total/e_hs_{}_{}.npy".format(ses, run)
                else:
                    run_path = save_dir + "/total/hs_{}_{}.npy".format(ses, run)
                rs = np.load(run_path)
                #print(rs.shape)
                hs = np.concatenate((hs, rs), axis=0)
            old = hs.shape
            #hs = np.delete(hs, wd_id_droppeds[ses], axis=0)
            print(old, hs.shape)
            #hs_path = save_path + "/hs_{}.npy".format(ses)
            #np.save(hs_path, hs)
            pbar.update()
...
if __name__ == '__main__':
    global tokenizer
    print('start at', maket(int(time.time())))
    
    for i in [36,37]:
        step = "iter_" + "{:07d}".format(i*10000)

        # Load model directly
        tokenizer = AutoTokenizer.from_pretrained("/data/minghua/hf_output/20240116-1900/"+step)
        model = AutoModelForCausalLM.from_pretrained("/data/minghua/hf_output/20240116-1900/"+step)
        for layer in range(33):
            main(step, layer, model)
    
    
    #filter("iter_0010000", 0, 3)
    print('finish at', maket(int(time.time())))
